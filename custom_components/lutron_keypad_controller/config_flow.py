...
_A3='led_bindings'
_A2='device_not_found'
_A1='already_configured'
_A0='lutron_not_loaded'
_z='model_number'
_y='controller'
_x='remote'
_w='hybrid'
...
LUTRON_TYPE_FUZZY=[('aliss',KEYPAD_ALISEE),(_v,KEYPAD_ALISEE),(_T,KEYPAD_PALLADIOM),(_U,KEYPAD_SUNNATA),(_w,KEYPAD_SEETOUCH_HYBRID),(_V,KEYPAD_SEETOUCH),(_W,KEYPAD_TABLETOP),(_X,KEYPAD_PICO),(_x,KEYPAD_PICO),(_J,KEYPAD_SEETOUCH)]
BUTTON_TYPE_KEYWORDS={_J,_X,_x,_V,_U,'aliss',_v,_T,_W,_w}
...
def _build_layout_from_button_devices(candidates,area_name,device_name):
	...
	C=_A;D=_A;F={};G={}
	for H in I:
		E=H.get(_B,'');J=E.lower();A=_resolve_btn_num(H);K=H.get(_Z)
		...
		if A is _A:continue
		if J.endswith((_a,_b,_c,_d))or _RAISE_NAME_RE.search(E):C=A
		elif J.endswith((_e,_f,_g,_h))or _LOWER_NAME_RE.search(E):D=A
		M=_strip_engraving(E,area_name,device_name)
		if M:F[str(A)]=M
	N=[A for A in B if A not in(C,D)];_LOGGER.debug('button_devices layout: %d total, configurable=%s raise=%s lower=%s names=%s leap_map=%s',len(B),N,C,D,F,G);return{_K:B,_O:N,_P:C,_Q:D,_I:F,_i:G}
def _build_layout_from_inline_buttons(buttons_list,area_name,device_name,device_full_name=''):
	...
	for H in buttons_list:
		...
		if J.endswith((_a,_b,_c,_d))or _RAISE_NAME_RE.search(E):C=B
		elif J.endswith((_e,_f,_g,_h))or _LOWER_NAME_RE.search(E):D=B
	...
	A=sorted(set(A));N=[A for A in A if A not in(C,D)];_LOGGER.debug('inline-buttons layout: %d total, configurable=%s raise=%s lower=%s names=%s leap_map=%s',len(A),N,C,D,F,G);return{_K:A,_O:N,_P:C,_Q:D,_I:F,_i:G}
def _build_layout_from_bridge_buttons(candidates,area_name,device_name,has_raise_lower=True):
	...
	for F in candidates:
		...
		if J:
			if L.endswith((_a,_b,_c,_d))or _RAISE_NAME_RE.search(G):A=E
			elif L.endswith((_e,_f,_g,_h))or _LOWER_NAME_RE.search(G):B=E
	...
	D=sorted(set(D));N=[C for C in D if C not in(A,B)];_LOGGER.debug('bridge.buttons layout: %d total, configurable=%s raise=%s lower=%s names=%s',len(D),N,A,B,H);return{_K:D,_O:N,_P:A,_Q:B,_I:H,_i:{}}
def _detect_button_layout(hass,serial,keypad_type,device_name='',area_name='',device_id='',device_data=_A):
	...
	for E in _iter_lutron_bridges(hass):
		...
		if B is _A:continue
		D=str(B.get(_H,''))or C;_LOGGER.debug('Device serial=%s on bridge %s — type=%r model=%r device_id=%s inline_buttons=%d button_devices_total=%d',A,type(E).__name__,B.get(_G),B.get(_j),D,len(B.get(_E,[])),len(F));K=B.get(_E,[])
		...
		_LOGGER.warning('Device serial=%s (type=%r model=%r device_id=%s) found on bridge but carries no button data (button_devices=%d, inline_buttons=0, bridge.buttons=%d). Full device info: %s',A,B.get(_G),B.get(_j),D,len(F),len(L),B);return{}
...
def _infer_lip_keypad_type(model):
	A=(model or'').lower()
	if _T in A:return KEYPAD_PALLADIOM
	if _U in A:return KEYPAD_SUNNATA
	if _V in A or'see touch'in A:return KEYPAD_SEETOUCH
	if'alisse'in A or'alise'in A:return KEYPAD_ALISEE
	if _W in A or'table top'in A:return KEYPAD_TABLETOP
	if _X in A:return KEYPAD_PICO
	return KEYPAD_GENERIC
def _lip_button_info(hass):
	B={}
	try:
		F=hass.data.get(_k)or{}
		for G in F.values():
			C=getattr(G,_y,_A)
			if C is _A:continue
			for H in getattr(C,'areas',[])or[]:
				for D in getattr(H,'keypads',[])or[]:
					...
					for A in getattr(D,_E,[])or[]:
						...
						I[J]={_R:(getattr(A,_R,'')or'').strip(),_G:getattr(A,'button_type','')or'',_l:getattr(A,_l,'')or''}
	except Exception:return{}
	return B
def _discover_lip_keypads(hass):
	H=hass;T=dr.async_get(H);U=er.async_get(H);V=_lip_button_info(H);P=[]
	for B in T.devices.values():
		C=next((str(A[1])for A in B.identifiers if A[0]==_k),_A)
		if C is _A:continue
		D={}
		for I in er.async_entries_for_device(U,B.id):
			if I.domain!='event':continue
			Q=re.search('(\\d+)$',I.entity_id)
			if Q:A=int(Q.group(1));D[A]=I.original_name or I.name or f"Button {A}"
		if not D:continue
		J=sorted(D);R=V.get(C,{});K={};F=G=_A
		for A in J:
			N=R.get(A,{});L=N.get(_l,'')
			if L==_m:F=A
			elif L==_n:G=A
			if L in(_m,_n):K[str(A)]=L
			elif N.get(_R):K[str(A)]=N[_R]
			else:K[str(A)]=D[A]
		if F is _A and G is _A and not R:G=18 if 18 in D else _A;F=19 if 19 in D else _A
		W=[A for A in J if A not in(F,G)];O=B.model or'';M=''
		if B.area_id:
			S=ar.async_get(H).async_get_area(B.area_id)
			if S:M=S.name
		if B.name_by_user:E=B.name_by_user
		elif M:E=f"{M} keypad {C}"
		else:E=B.name or f"keypad {C}"
		P.append({_L:f"lip_{C}",_B:E,_C:f"{E} ({O or _J}, {len(J)} buttons)",'data':{_B:E,'backend':_S,'lip_id':C,_H:B.id,CONF_DEVICE_SERIAL:f"lip_{C}",CONF_DEVICE_NAME:E,CONF_AREA_NAME:M,CONF_KEYPAD_TYPE:_infer_lip_keypad_type(O),_z:O,_K:J,_I:K,_O:W,_P:F,_Q:G}})
...
class LutronKeypadsConfigFlow(config_entries.ConfigFlow,domain=DOMAIN):
	...
	async def async_step_user(A,user_input=_A):
		C=bool(A.hass.config_entries.async_entries(_M));B=bool(A.hass.config_entries.async_entries(_k))
		if not C and not B:return A.async_abort(reason=_A0)
	...
	async def async_step_source(A,user_input=_A):
		...
		if C is not _A:
			if C[B]==_S:return await A.async_step_lip()
			return await A.async_step_caseta()
		return A.async_show_form(step_id=B,data_schema=vol.Schema({vol.Required(B,default=_o):vol.In({_o:'Caséta / RA2 Select (lutron_caseta)',_S:'Homeworks QS / RadioRA (lutron_lip)'})}))
	async def async_step_lip(A,user_input=_A):
		...
		if not B:return A.async_abort(reason=_A1)
		E={}
		if D is not _A:
			H=D.get(_J);C=next((A for A in B if A[_L]==H),_A)
			if C is _A:E['base']=_A2
		...
		return A.async_show_form(step_id=_S,data_schema=vol.Schema({vol.Required(_J):vol.In({A[_L]:A[_C]for A in B})}),errors=E,description_placeholders={'count':str(len(B))})
	async def async_step_caseta(A,user_input=_A):
		F='device_serial';C=user_input;G=A.hass.config_entries.async_entries(_M)
		if not G:return A.async_abort(reason=_A0)
		...
		if C is not _A:
			...
			if A._selected_device is _A:D['base']=_A2
		...
		return A.async_show_form(step_id=_o,data_schema=vol.Schema({vol.Required(F):vol.In(I)}),errors=D,description_placeholders={'count':str(len(B))})
	async def async_step_confirm(B,user_input=_A):
		...
		if G is not _A:J=G.get(_B,I).strip();return B.async_create_entry(title=J,data={_B:J,CONF_DEVICE_SERIAL:H,CONF_DEVICE_NAME:D,CONF_AREA_NAME:C,CONF_KEYPAD_TYPE:F,_p:E,_z:A.get(_j,''),_H:A.get(_H,''),**B._detected_layout})
		...
		return B.async_show_form(step_id='confirm',data_schema=vol.Schema({vol.Required(_B,default=I):str}),description_placeholders={'area':C or'—','device_name':D,'keypad_type':F,_D:H,_p:E,'button_count':L})
	async def async_step_manual(B,user_input=_A):
		A=user_input;D={}
		if A is not _A:
			C=A.get(CONF_DEVICE_SERIAL,'').strip()
			if not C:D[CONF_DEVICE_SERIAL]='serial_required'
			else:await B.async_set_unique_id(C);B._abort_if_unique_id_configured();E=A.get(_B,C).strip();return B.async_create_entry(title=E,data={_B:E,CONF_DEVICE_SERIAL:C,CONF_DEVICE_NAME:A.get(CONF_DEVICE_NAME,''),CONF_AREA_NAME:A.get(CONF_AREA_NAME,''),CONF_KEYPAD_TYPE:KEYPAD_GENERIC,_p:''})
	...
	async def async_step_controller(A,user_input=_A):await A.async_set_unique_id(_y);A._abort_if_unique_id_configured();return A.async_create_entry(title='Lutron Keypad Controller',data={'_controller':True})
	@staticmethod
	@callback
	def async_get_options_flow(config_entry):return LutronKeypadsOptionsFlow()
_ACTION_OPTIONS=[{_q:A,_C:B}for(A,B)in ACTION_TYPE_LABELS.items()]
class LutronKeypadsOptionsFlow(config_entries.OptionsFlow):
	...
	def _get_configurable(A):return[A for A in A._get_all_buttons()if not A[_r]and not A[_s]]
	def _get_raise_lower_note(C):
		A=[]
		for B in C._get_all_buttons():
			if B[_r]:A.append(f"Button {B[_F]} (Raise)")
			elif B[_s]:A.append(f"Button {B[_F]} (Lower)")
		if not A:return''
	...
	def _default_entity(B,cfg,multiple):
		...
		return A or''
	async def async_step_init(A,user_input=_A):B=A.config_entry.entry_id;return A.async_show_menu(step_id='init',menu_options=[_E,'license'],description_placeholders={'panel_url':f"/lutron-keypads?entry={B}",_t:A.config_entry.title})
	...
	async def async_step_buttons(A,user_input=_A):
		...
		if D is not _A:
			for C in E:
				...
				if G not in ACTION_TYPES_NEEDING_ENTITY:A._buttons_config[B][CONF_ACTION_TARGET]=[];A._buttons_config[B][CONF_LED_ENTITY]='';A._buttons_config[B][_u]=''
			for C in A._get_all_buttons():
				if C[_r]:A._buttons_config[C[_F]]={_C:_m,CONF_ACTION_TYPE:ACTION_RAISE}
				elif C[_s]:A._buttons_config[C[_F]]={_C:_n,CONF_ACTION_TYPE:ACTION_LOWER}
		...
		return A.async_show_form(step_id=_E,data_schema=vol.Schema(F),description_placeholders={_t:L,'raise_lower_note':A._get_raise_lower_note(),_A3:A._get_led_bindings_note()})
	async def async_step_entities(B,user_input=_A):
		...
		if C is not _A:
			for J in I:
				...
				if D==ACTION_STATEFUL_SCENE:B._buttons_config[A][CONF_LED_ENTITY]=C.get(f"button_{A}_led",'');L=C.get(f"button_{A}_scene_group",'');B._buttons_config[A][_u]=L.strip()if isinstance(L,str)else''
